...
const EventSchema = new mongoose.Schema({
    ...
    date: {
        ...
        validate: {
          ...
          message: "Event date must be at least 1 minute in the future (UTC time)"
        }
      },
    location: {
        type: String,
    ...
    category: {
        type: String,
        enum: ["workshop", "seminar", "social", "hackathon", "cultural"],
        required: true
    },
    organizer: {
...
EventSchema.pre("save", async function(next) {
    if (this.isNew) { // Only check for new events
